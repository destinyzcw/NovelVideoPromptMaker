...
Use these schemas rather than adapting prompt habits from LTX, Kling, Runway, or other models.
...
For storyboard production, prefer FL2VA when the rendered K0/K1 frames are the contract. Prefer
Ref2VA when reusable identity, voice, motion, or style assets matter more than exact endpoints.

## 3. Base-mode prompt contract
...
For generation tasks, MiniMax recommends roughly 350–500 English words, adjusted for dialogue
density. Before `[Shot 1]`, establish the overall style in one or two sentences. Then describe
each shot in playback order, inserting labels at first appearance and wherever their role applies.
