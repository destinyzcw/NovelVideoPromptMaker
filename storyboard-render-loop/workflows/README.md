# LTX-2.3 FLF2V ComfyUI workflows (fp8)

Two ready-to-run **First-Last-Frame → Video** (FLF2V) workflows for LTX-2.3, wired for
the **fp8** diffusion checkpoint **and the fp8 Gemma-3 12B text encoder**. FLF2V is the
mode the storyboard skills target: each shot is a keyframe chain `K0 → K1 → … → Kn`, and
each adjacent pair `Ki → Ki+1` is one short clip generated by feeding the two keyframes as
the first and last frame plus the per-segment motion prompt.
